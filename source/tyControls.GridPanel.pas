...
type
  { One cell of a TTyGridPanel. A transparent, windowed container: it paints NOTHING
    (no themed background/border) and only positions + clips + pads its dropped children.
    Col/Row are its grid coordinates (set by the grid, streamed so a loaded form re-seats
    it). Padding insets its content on all sides. Mirrors TTyTabSheet's designer-container
    ControlStyle, but stays VISIBLE (all cells show at once, unlike a tab page).

    Lives in the SAME unit as TTyGridPanel on purpose: the Lazarus designer adds the
    dropped component's unit to the form's uses, but NOT the units of sub-components the
  ...
  public
    ...
    { Transient (NON-published, never streamed): True on a cell the owning grid seeded as
...
initialization
  { Non-field nested cells are instantiated by the streamer via class name. }
