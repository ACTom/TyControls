...
procedure TTyLabel.WrapText(const AText: string; AMaxWidthPx: Integer;
  ACanvas: TCanvas; ALines: TStrings);
{ Greedy line-break that understands both scripts. Western words break at
  spaces (and are rejoined with a single space, collapsing runs). CJK text
  carries no spaces, so each ideograph / kana / hangul syllable is its own
  break opportunity -- without this a Chinese caption is one unbreakable word
  and overflows a narrow label instead of wrapping (see TestWordWrapCJK).
  Walks UTF-8 by codepoint so multi-byte glyphs are never split mid-sequence. }
var
  cur, buf: string;
