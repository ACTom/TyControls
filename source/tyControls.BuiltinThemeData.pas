...
begin
  Result :=
    ...
    '' + LineEnding +
    '    /* NO --surface / --on-surface pin here, and that is a decision, not an omission —' + LineEnding +
    '       the dark block carries one (see there) and this block deliberately does not.' + LineEnding +
    '       In light the pin would be a NO-OP for the surface: aero''s --field-bg is #FFFFFF' + LineEnding +
    '       and the base''s light --surface is #FFFFFF, so the fallback page sheet already IS' + LineEnding +
    '       aero''s own page sheet. Writing `--surface: var(--field-bg)` here would change no' + LineEnding +
    '       resolved byte and would only add a line to keep in sync. The ink is a near-no-op' + LineEnding +
    '       for a different reason: base light --on-surface #1F2937 (31,41,55) is already a' + LineEnding +
    '       cold slate two levels off aero''s own --ink #1E2B3A (30,43,58) — 1.4 luma and 3' + LineEnding +
    '       chroma apart, inside the palettes'' own rounding, and nothing on screen separates' + LineEnding +
    '       them. Pinning it would move real bytes for no visible gain, so light stays as it' + LineEnding +
    '       resolves today. The dark side had neither escape: its --field-bg is #141C27 and' + LineEnding +
    '       its ink b-r is +23 against the base''s +6. */' + LineEnding +
    '}' + LineEnding +
    '}' + LineEnding +
    '' + LineEnding +
    '/* ---- DARK palette: Win7 dark colorization — deep blue-black glass, wet gloss kept.' + LineEnding +
    '   This block also PINS the base layer''s dark seeds (--surface / --on-surface, at the foot' + LineEnding +
    '   of the block) to aero''s own cold family, so fallback controls and aero''s own chrome read' + LineEnding +
    '   as ONE dark window. It did not always: leaving the seeds unpinned left every unstyled' + LineEnding +
    '   surface on the base''s neutral #1E1E1E, which is the same off-family neutral the chrome' + LineEnding +
    '   fix removed from the bars, only bigger — see the seed comment for the full story.' + LineEnding +
    '   Ink is pale slate, not white: glass, not console.' + LineEnding +
    ...
    '' + LineEnding +
    '    /* PAGE SHEET seeds — the third and last off-family neutral, and the biggest one. The' + LineEnding +
    '       chrome fix above retuned the BARS; these two retune everything the skin does not' + LineEnding +
    '       style at all. --surface is the base layer''s fill for TyPanel, TyBevel, TyScrollBox,' + LineEnding +
    '       TyExPanel, TyScrollContent, TyImageView, TyChart … (one rule, light.tycss); leaving' + LineEnding +
    '       it unpinned handed every one of them the base''s dark seed #1E1E1E — a NEUTRAL' + LineEnding +
    '       near-black (chroma 0) sitting at luma 30.0 against this skin''s own form wash at' + LineEnding +
    '       luma 29.8. Same lightness, different hue: the eye gets a colour boundary with no' + LineEnding +
    '       depth cue to explain it, and the containers demo showed two backgrounds side by' + LineEnding +
    '       side — aero''s cold blue-black on the form/group boxes/cards, flat neutral on the' + LineEnding +
    '       bevels, paint panel, scroll wells and the panels hosting the tool bar.' + LineEnding +
    '' + LineEnding +
    '       Why LIGHT needs no such pin, and is deliberately left alone: there the coincidence' + LineEnding +
    '       already holds. aero''s light --field-bg is #FFFFFF and the base''s light --surface is' + LineEnding +
    '       #FFFFFF — the same value, so the fallback page sheet already IS aero''s own page' + LineEnding +
    '       sheet, and light resolves stay byte-identical. The dark block broke that coincidence' + LineEnding +
    '       when it gave --field-bg its own cold well (#141C27) and left --surface behind.' + LineEnding +
    '       Restoring it is the whole fix: point the seed at the well the skin already chose.' + LineEnding +
    '       (The secondary reason light reads fine even so: white sits 20 luma ABOVE the wash,' + LineEnding +
    '       so it reads as a lighter sheet on a tinted ground — a legible depth relationship.' + LineEnding +
    '       Dark''s neutral sat at the SAME luma as the wash, which is a clash and nothing else.)' + LineEnding +
    '' + LineEnding +
    '       Luma is preserved on purpose — 26.9 against the old 30.0. Everything in this block' + LineEnding +
    '       was tuned against a page sheet at ~30 (the tab ramp below cites it; the ink deltas' + LineEnding +
    '       were measured on it), so moving the HUE alone fixes the reported defect and re-opens' + LineEnding +
    '       none of that tuning. Every derived token re-derives correctly without help, because' + LineEnding +
    '       darken() is a per-channel multiply: it scales a cold value to a darker cold value' + LineEnding +
    '       and can no longer manufacture a neutral out of a coloured seed.' + LineEnding +
    '' + LineEnding +
    '       --on-surface is the matching ink: base dark #E5E7EB is near-neutral (b-r +6) where' + LineEnding +
    '       this skin''s own body ink is pale cool slate (b-r +23), so captions on a fallback' + LineEnding +
    '       panel were drawn in a different white from captions on the form. var(), not a copy,' + LineEnding +
    '       so a retune of the well or the ink carries the fallbacks along. */' + LineEnding +
    '    --surface:        var(--field-bg);   /* #141C27 — the page sheet IS the field well, as in light */' + LineEnding +
    '    --on-surface:     var(--ink);        /* #D9E4F0 — fallback captions in the skin''s own ink */' + LineEnding +
    '' + LineEnding +
    ...
    '       var(--surface): the dark page sheet (#141C27, luma ~27). Dark chrome lifts off the' + LineEnding +
    ...
    '       ink flips to the accent, ~97 luma above the sheet). Dark ink #D9E4F0 keeps' + LineEnding +
...
begin
  Result :=
    ...
    'TyButton, TyDropDownButton, TyMenuButton, TyColorButton, TySpeedButton, TyGlyphContainerButton, TyRibbonAppMenu, TyButtonGroup, TyUpDown {' + LineEnding +
    '  background: var(--field);' + LineEnding +
    '  color: var(--ink);' + LineEnding +
    ...
    '  padding: 5px 10px;' + LineEnding +
    '}' + LineEnding +
    'TyButton:hover, TyDropDownButton:hover, TyMenuButton:hover, TyColorButton:hover, TySpeedButton:hover, TyGlyphContainerButton:hover, TyRibbonAppMenu:hover, TyButtonGroup:hover, TyUpDown:hover {' + LineEnding +
    '  background: var(--surface);' + LineEnding +
    '  color: var(--ink);' + LineEnding +
    '  border: 1px solid var(--line);' + LineEnding +
    '}' + LineEnding +
    'TyButton:active, TyDropDownButton:active, TyMenuButton:active, TyColorButton:active, TySpeedButton:active, TyGlyphContainerButton:active, TyRibbonAppMenu:active, TyButtonGroup:active, TyUpDown:active {' + LineEnding +
    '  background: var(--pressed);' + LineEnding +
    '  color: var(--ink);' + LineEnding +
    '  border: 1px solid var(--line-field);' + LineEnding +
    '}' + LineEnding +
    'TyButton:focus, TyDropDownButton:focus, TyMenuButton:focus, TyColorButton:focus, TySpeedButton:focus, TyGlyphContainerButton:focus, TyRibbonAppMenu:focus, TyButtonGroup:focus, TyUpDown:focus  { outline: 2px var(--accent); outline-offset: 1px; }' + LineEnding +
    'TyButton:disabled, TyDropDownButton:disabled, TyMenuButton:disabled, TyColorButton:disabled, TySpeedButton:disabled, TyGlyphContainerButton:disabled, TyRibbonAppMenu:disabled, TyButtonGroup:disabled, TyUpDown:disabled {' + LineEnding +
    '  background: var(--surface);' + LineEnding +
    '  color: var(--muted);' + LineEnding +
    '  border: 1px solid var(--line);' + LineEnding +
    '}' + LineEnding +
    ...
    '}TyButton.primary:hover, TySpeedButton.primary:hover, TyGlyphContainerButton.primary:hover, TyRibbonAppMenu.primary:hover, TyButtonGroup.primary:hover, TyUpDown.primary:hover  { background: darken(var(--accent), 8);  color: on(var(--accent)); border: 1px solid darken(var(--accent), 8); }TyButton.primary:active, TySpeedButton.primary:active, TyGlyphContainerButton.primary:active, TyRibbonAppMenu.primary:active, TyButtonGroup.primary:active, TyUpDown.primary:active { background: darken(var(--accent), 16); color: on(var(--accent)); border: 1px solid darken(var(--accent), 16); shadow: 0 0 0 #00000000; }TyButton.primary:disabled, TySpeedButton.primary:disabled, TyGlyphContainerButton.primary:disabled, TyRibbonAppMenu.primary:disabled, TyButtonGroup.primary:disabled, TyUpDown.primary:disabled {' + LineEnding +
    ...
    '  padding: 5px 10px;' + LineEnding +
    '  shadow: 0 1 2 #00000024;' + LineEnding +
    '}TyButton.danger:hover, TySpeedButton.danger:hover, TyGlyphContainerButton.danger:hover, TyRibbonAppMenu.danger:hover, TyButtonGroup.danger:hover, TyUpDown.danger:hover  { background: darken(var(--danger), 8);  color: on(var(--danger)); border: 1px solid darken(var(--danger), 8); }TyButton.danger:active, TySpeedButton.danger:active, TyGlyphContainerButton.danger:active, TyRibbonAppMenu.danger:active, TyButtonGroup.danger:active, TyUpDown.danger:active { background: darken(var(--danger), 16); color: on(var(--danger)); border: 1px solid darken(var(--danger), 16); shadow: 0 0 0 #00000000; }TyButton.danger:disabled, TySpeedButton.danger:disabled, TyGlyphContainerButton.danger:disabled, TyRibbonAppMenu.danger:disabled, TyButtonGroup.danger:disabled, TyUpDown.danger:disabled {' + LineEnding +
