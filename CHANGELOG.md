...
- **仪表控件(`TTyGauge`)** —— 新增数值仪表控件:线性横 / 竖条、开口弧形(速度表)、整环四种样式,
  数值变化带缓动动画,可显示格式化数值;颜色由主题的 `TyGauge` / `TyGaugeFill` 规则驱动。跨平台像素一致。
- **关于对话框(`TTyAboutDialog`)** —— 新增可复用的自绘“关于”对话框:自定义标题栏 + accent 头部
  (应用名 + 版本),下方是描述 / 版权 / 许可证 / 可点击的主页链接;**未填写的字段自动隐藏**,对话框
  收缩到只显示已有内容。控件的设计期 `About` 属性也改用它,外观与随库发布的对话框完全一致。
- **选择文件夹对话框** —— 顶部新增路径输入框:实时显示当前选中的文件夹,也可**直接输入或粘贴一个
  路径**并回车,直接定位到该文件夹。
