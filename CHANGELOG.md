...
- **`window-shadow: false` 时窗口失焦会盖出一整条 Windows 经典标题栏**(论坛反馈)。
  ...
  **带 `WS_CAPTION` 的窗口**的标题栏——而且不给窗口发任何绘制消息(消息探针上只有
  ...
  与最大化不受影响。
- **`window-shadow: false` 的窗口拖不动边框改大小**(论坛反馈)。这个模式为了不让系统画出
  一圈灰白旧边框,把整个窗口矩形都给了客户区——代价是铺满客户区的内容宿主盖住了每一个像素,
  鼠标事件全被这个子窗口接走,**窗体自己一次都没被问过命中测试**,能报出"左边/右下角"的
  那段映射自然永远跑不到。量出来的对比很干净:悬停右下角,开阴影时窗体收到 13 条
  `WM_NCHITTEST` + 13 条 `WM_NCMOUSEMOVE(HTBOTTOMRIGHT)`,关阴影时**两样都是 0**。
  现在内容宿主在边缘一圈内回答"这不是我的",系统就会接着问下面的窗体。窗口的样子一个像素
  没变(逐点扫过边缘,与修复前完全一致),窗口内部的点击也照旧归内容宿主。
- **标题栏偶尔高了一倍**(论坛反馈,250% 缩放)。窗体尺寸变化的消息可能**早于**系统的
  `WM_DPICHANGED` 到,于是标题栏按旧 DPI 算了一次高度、又没等到重算,叠成两倍。现在标题栏的
  高度自适应从字体的 `PixelsPerInch` 重新推出(幂等,与消息先后无关),两条路径殊途同归。
- **最小化 / 最大化 / 关闭按钮在 250% 下显得"发糊发粗"**(论坛反馈)。这几个字形的描边宽度被
  缩放了**两次** —— 一次在算笔宽时、一次在绘制时,于是 250% 下画成了 2–3 倍粗。现在只缩一次。

### 新增 — 图像列表大改:矢量列表**就是**图像列表

- **`TTyVirtualImageList` 现在继承 LCL 的 `TCustomImageList`** —— 它既保留"任意像素尺寸按需渲染
  矢量/图标字形"的老本事,又是一个货真价实的图像列表,能直接赋给任何吃 `TCustomImageList` 的地方
  (含 LCL 原生控件和第三方控件)。反过来,本库每一个吃图标的控件,`Images` 都放宽成
  `TCustomImageList`:控件在运行时自己判断——是我们的列表就走矢量那条更优的路,是普通 LCL 列表
  就按它的分辨率画。一套代码,两条路径,平台无关。
- **图标改用"名字"作键,铺开到几乎所有带图标的控件。** `TTyImage`、表头列(连带网格列)、
  标签页、树节点(普通 + 选中两个槽)、`TTyComboBoxEx` 行(图标 / 角标 / 选中三个槽)、列表视图项,
  都新增了与 `ImageIndex` 配对的 `ImageName`(树节点还有 `SelectedName`,组合框行还有
  `OverlayImageName` / `SelectedImageName`)。**名字是持久状态,序号是它的视图**:你重排图片列表,
  控件的图标跟着自己的名字走,而不是被挤进那个槽的图换掉。对普通 LCL 列表(没有名字)则名字自动
  失效、退回按序号——两不耽误。序号只在名字无法承载时才写进 `.lfm`,所以重排后再打开不会被旧序号
  覆盖掉名字。
- **内置 Lucide 图标包现在也是一个可直接拖放的图像列表 `TTyLucideImageList`。** 拖上窗体、
  赋给任意控件的 `Images` 即可,不用再单独接字体——它内部已连上共享的 Lucide 字体。名字表默认为空:
  填你实际用到的图标名(或用图标浏览器挑),每个名字就成一张图,可按序号或按名字引用。仍是可选单元,
  不用就不进你的程序。
- **图标浏览器对话框 `TTyIconBrowserDialog`** —— 在 2000 多个图标里靠肉眼翻下拉框找不到东西,
  现在有了带搜索的可视化浏览器;每张图同时显示**名字和 ImageIndex**,按名字用和按序号用的人都能直接抄。
...
- **`TTyTabStrip.TabHeight`**:`0` 保留"不显示标签条"(示例在运行时用它),**自动**改到负数上,
  ...
  提前返回,于是取值缩了、标签条没动。
- **所有 `Images` 属性统一为 LCL 的 `TCustomImageList`,接受任意标准图像列表。** 起因是
  `TTyVirtualImageList` 现在**本身就是一个图像列表**(见下方"图像列表大改")——它既能按需渲染
  矢量,又是合法的 `TCustomImageList` 后代。于是 `TTyHeader.Images`、网格、列表视图、树、菜单、
  下拉框等每一个吃图标的控件,`Images` 都能同时接住**本库的矢量列表**和**普通 LCL 列表**:是我们的
  就走按需渲染那条更清晰的路,是别人的就用它自己的分辨率画。网格不再需要自带第二份列表绕开谁。
