...
begin
  ...
  w1 := B.Width;
  h1 := B.Height;
  AssertEquals('a single glyph is a circle', h1, w1);
  B.Value := 150;   // '99+'
...
begin
  ...
  try
    ...
    Reread := TBGRABitmap.Create(Bmp);
    try
      // The pill fills the control's own rect (the badge is always its natural size).
      bb := BlueCoreBBox(Reread);
      AssertTrue('themed accent pill painted', (bb.Right > bb.Left) and (bb.Bottom > bb.Top));
      AssertTrue('pill spans the control width (rounded ends aside)',
        (bb.Right - bb.Left) >= B.Width - 2);
      AssertTrue('pill spans the control height',
        (bb.Bottom - bb.Top) >= B.Height - 2);
