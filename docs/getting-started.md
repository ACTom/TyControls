...
| `edit/` | `TTyEdit` 文本输入、选区、剪贴板、焦点态 | `lazbuild examples/edit/edit_example.lpi` |
...
1. **`TTyComboBox` 无下拉弹层** — v1/v1.1 的下拉框点击后在原地循环选项,不弹出列表框。浮层弹出为 Tier-2 规划项。

2. **`TTyEdit` 无横向滚动** — 当文本宽度超过控件宽度时,右侧内容不可见;也不支持词级跳转(Ctrl/Cmd+左右键透传给父窗口,不做词边界计算)。

3. **窗框 Tier-2 缺口** — `TTyFormChrome` 采用跨平台手动实现,Windows Aero Snap 贴边分屏、Windows DWM 原生投影阴影、macOS 原生红绿灯按钮等原生窗口行为尚未实现。macOS 原生阴影与跨屏 DPI 重缩放已在 v1.1 解决。

4. **设计期渲染近似** — Lazarus 窗体设计器显示的是原生(未换肤)框架;完整自绘窗框只在运行期呈现。这是此类皮肤库的通常行为。
