...
TTyShape 是 TyControls 的**通用矢量形状控件**(TShape 的主题化重制),继承自 `TTyGraphicControl`(叶子图形控件,无焦点 / 子控件)。它用 BGRABitmap 的 `Canvas2D` 抗锯齿绘制一个矢量形状——矩形、圆角矩形、正方形、椭圆、圆、三角形、菱形或一条对角线——**填充色取自解析后的 `TyPanel` 背景,描边取自 `TyPanel` 边框**。控件不硬编码任何颜色:填充 / 边框全部来自解析后的 `TyPanel` 样式,因此形状跟随当前主题。要单独给某个形状换色,用 `StyleClass` / `StyleOverride`(例如 `StyleOverride := 'background: #E11; border-color: #700;'`)。
...
- **填充** = `TyPanel` 的 `background`(仅当为纯色且非全透明时填充);
- **描边** = `TyPanel` 的 `border-color`,线宽 `max(1, Scale(border-width))`;`border-style: none`
  与 `border-width: 0` 都会关掉描边(判据 `TyBorderVisible`,与库内其它控件一致)。**例外**:
  `tskLine` 的那条线本身就是形状而非外框,始终绘制;
- **圆角矩形** 的圆角取自 `TyPanel` 的 `border-radius`。
...
因填充 / 边框取自 `TyPanel` 主题规则,更换主题即更换形状颜色。要单独调色,用 `StyleOverride`(例如 `'background: var(--accent); border-color: var(--border);'`)。
...
- **RoundRect 圆角来自主题:** 圆角半径 = `TyPanel` 的 `border-radius`(经 `Scale` 缩放,且不超过较短边的一半);主题半径为 0 时退化为普通矩形。
