...
| typeKey（轨道） | `TyToggleSwitch` |
| typeKey（旋钮子部件） | `TyToggleKnob` |
...
- **轨道：** 使用 `TyToggleSwitch` 样式规则绘制背景与边框；若主题未声明 `border-radius`，引擎自动使用控件高度的一半（胶囊形）。支持 `:hover`（Batch ④，悬停时轨道略深）、`:active`（ON 或按下）、`:focus`、`:disabled`。
- **旋钮：是独立子部件 typeKey `TyToggleKnob`（Batch ④，tier-a 着色面）。** 旋钮的填充色取 `TyToggleKnob` 的 `background`（默认 `#FFFFFF`），圆角取其 `border-radius`（默认 `12px`），不再借用 `TyToggleSwitch` 的 `color`。旋钮在 OFF 时位于轨道左侧，ON 时位于右侧；边距为 3 逻辑像素（DPI 缩放）。
- **旋钮圆角钳制：** `TyToggleKnob.border-radius` 经 `TyClampRadius` 钳到旋钮逻辑半径（`旋钮边长/2`）。默认 44×24 开关下旋钮半径约 9px，`Min(12, 9) = 9` → 仍是完整圆。把 token 调小即可得到圆角方块旋钮，超大值不会让旋钮越出自身轮廓。

### light.tycss 示例规则

```css
TyToggleSwitch {
  background: darken(--surface, 18%);  /* OFF 时轨道颜色 */
  color: #FFFFFF;                      /* （tier-b 字形墨色；本控件旋钮已改走 TyToggleKnob） */
...
2. **旋钮颜色来自子部件 typeKey `TyToggleKnob`（Batch ④）：** 旋钮的填充色与圆角取 `TyToggleKnob` 的 `background` / `border-radius`（默认白色、`12px`），不再借用 `TyToggleSwitch` 的 `color`。常见配置为 `TyToggleKnob { background: #FFFFFF; }`（白色旋钮）以在彩色轨道上提供对比。默认值与早前借用 `color` 渲染时一致，老主题升级后旋钮外观不变。
