...
2. **标题文字背景带：** 在顶边按 `Alignment` 定位（默认 `taLeftJustify` 靠左、`taCenter` 居中、`taRightJustify` 靠右）绘制一段与控件背景同色的矩形，遮住边框线（宽度 = 8px 边距 + 文字宽度 + 8px 边距）；若主题未声明 `background`，使用黑色作为后备填充（不美观，请在主题中声明 `background`）；
3. **标题文字：** 以 `TextColor` 在标题带内绘制，水平对齐随 `Alignment`。
