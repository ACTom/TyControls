...
- **而且只有一个坐标空间:设备像素。** 两个命中函数收的 `(AX, AOriginX, APPI)` 就是绘制端
  交给 `Span()` 的同一组参数,所以命中回答的边界**就是**画出来的那条边界。它们曾经先把
  设备坐标折算成逻辑像素再比较,同一条边界因此被取整两次:实测 PPI 120/144/168/192 ×
  列宽 100..109 共 50 组里有 22 组,某一列的最后一个设备像素会被命中成下一列。
  `DetermineSplitterIndex` 的 ±3/5 容差是个**逻辑**量(它描述的是手感,不是像素),
  在函数内部按 APPI 放大,所以高分屏上抓手的物理宽度不变。
...
| 部位 | 镜像后 |
|---|---|
| report 列轴 | 第 0 列贴视口**右**缘,其余向左排;表头格、报表行、格线、拖列浮标同源 |
| 列分隔线 / 拖宽抓手 | 是一列的**左**缘;向左拖是**变宽** |
| 表头 | 标题靠右,排序三角与列图标换到单元格的阅读起点(右侧) |
| 行内容 | 复选框、图标在主列的**右**端,文字向左让开它们 |
| 图标 / 小图标 / 平铺 / 列表 四种流式布局 | 单元格从右往左铺;`lvsList` 的第一条竖列贴右缘 |
| 分组布局 | 与平铺同一次反射 —— 两份算术,一个反射点 |
| 框选 | 跟着单元格走 |
| `←`/`→` | 跟着眼睛:`→` 走到物理右边那一格,也就是**前一项**。`Home`/`End`/`PageUp`/`PageDown` 是逻辑首尾,不翻 |

**不动的,而且是有意的:**

- **竖滚动条仍在右缘。** 反射轴是**视口** `[0, ViewportW)`,滚动条槽在视口之外;把它挪到
  左边就要移动这条轴,于是本文件里每一处满宽表达式前面都要加同一个非零常量。
  `plans/2026-08-04-rtl-mirroring-scope.md` §5 第 7 条把"条在错的一边"排为**最显眼、
  因而最安全**的漏项。由 `tests/test.rtl.pas` 的 `TRtlExclusionTest` 钉住:
  滚动条的边与反射轴必须一起改。
- 容器不镜像子控件的 `Align`/`Anchors` 布局(全库规则,见 `docs/rtl.md`)。

**后代**:`TTyShellListView` 不覆写 `RtlLayout` —— 它只换了五个数据访问器和列宽分配,
一个 x 也没有自己算。加后代时先问这句话:**它自己算 x 吗?** 算,就覆写成 `False`
(与 `TTyListBox.RtlRowLayout` 同一条约定)。

守卫在 `tests/test.rtl.pas` 的 `TRtlListViewTest`:列命中逐像素扫过整条表头,
流式命中逐像素扫过整行,两者都与**渲染出来的像素**(格线、选中填充)对齐,
不是与它自己的几何函数对齐。
