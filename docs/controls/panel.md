...
- **右到左镜像：只翻 Caption，不翻子控件。** `BiDiMode := bdRightToLeft` 时 `Alignment` 按阅读序解释（默认 `taCenter`，居中是不动点，所以默认情况下什么都不变）。**子控件的 `Align`/`Anchors` 排布不镜像**：LCL 的对齐引擎在 `wincontrol.inc` 里只有 `ChildSizing` 的**表格**路径带 BiDi 分支（`:1551`，而 `TTyPanel` republish 了 `ChildSizing`，这一份是白捡的），`alLeft` 的子控件在原生 `TPanel` 上也还在左边。我们跟着不镜像，否则移植过来的窗体会与原生容器排得相反。见 [KNOWN_GAPS.md](../KNOWN_GAPS.md#bidirectional-right-to-left-text)。
