...
| `TabVisible`（published, 默认 `True`） | 是否在页签条上**显示本页的页签**——名字与规则都照 `TTabSheet.TabVisible`：设 `False` 页签消失（不占宽、不可点、键盘导航跳过),**页和内容都留着**;隐藏当前活动页的页签会自动切到最近的可见页(先向后再向前);代码里 `ActivePageIndex := 隐藏页` 依然合法——全部隐藏 + 程序翻页 = 向导模式;设计期所有页签照常显示(点不到就没法设计)。典型用途:按权限隐藏功能页。注意与 `Visible` 无关:`Visible` 是切页机制的内部状态,别手动设它 |
