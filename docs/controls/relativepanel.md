...
**与 LCL 锚点（AnchorSide）的分工**：静态的设计期布局请直接用 LCL 锚点——IDE 自带可视化锚点编辑器,锚点随 `.lfm` 流式,还能双边拉伸,这些本控件都不做也不打算做（规则只能 `SetRules` 从代码设,不流式）。本控件的位置是**运行时/代码生成的界面**：动态逐条 `SetRules` 一行一个关系、统一 `Spacing`、依赖环安全,比在代码里手拼 `AnchorSide` 四元组轻——以及从 Android/WinUI 移植布局时的心智对应。
