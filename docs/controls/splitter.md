...
| 属性 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `MinSize` | `Integer` | `30` | 被调整面板允许的最小轴向尺寸（拖动时钳制）；`SetMinSize` 写入时将负值夹到 `0`（已定义 `default 30`，参与 DFM 差量存储） |
| `ResizeStyle` | `TResizeStyle` | `rsUpdate` | 拖动反馈方式：`rsUpdate` = 拖动过程中实时更新目标尺寸；`rsLine` = 延迟到松开鼠标时才应用一次（当前实现不绘制实时预览线，见注意事项）（已定义 `default rsUpdate`） |
...
| 事件 | 类型 | 触发时机 |
|------|------|----------|
| `OnCanResize` | `TTySplitterCanResizeEvent` | 每次应用新尺寸**之前**触发；签名 `(Sender: TObject; var ANewSize: Integer; var AAccept: Boolean)`。可在 handler 中改写 `ANewSize` 以钳制目标尺寸，或把 `AAccept := False` 否决本次调整 |
...
- **尺寸钳制：** 新尺寸经 `TySplitterNewSize` 计算，下界为 `MinSize`，上界为父容器客户区可用尺寸减去分隔条自身厚度（`Parent.ClientWidth/ClientHeight - Width/Height`）。`SetMinSize` 会把负的 `MinSize` 夹到 `0`。
- **`rsLine` 目前无实时预览线：** `ResizeStyle = rsLine` 只是把实际调整**延迟到 `MouseUp`** 应用一次，拖动过程中**不**绘制 XOR/幽灵预览线（该增强被标注为延后项）。若需要拖动时立即看到面板变化，请用默认的 `rsUpdate`。
...
- **DFM 序列化：** `MinSize`（`default 30`）、`ResizeStyle`（`default rsUpdate`）、`Align`（`default alLeft`）均声明了默认值，等于默认值时不写入 `.lfm`/`.dfm`。
