...
继承自 [TTyCustomControl](../../source/tyControls.Base.pas)(带自有窗口句柄的图形控件),按库标准的 `TTyPainter` 方式绘制。复用 `'TyGauge'` 主题(方块边框 / 准星文字色),**无需新增 `.tycss` 规则**。
...
| typeKey | `'TyGauge'`(边框 `BorderColor` / `BorderWidth`、准星 `TextColor`)|
