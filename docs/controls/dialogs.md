...
弹出 HSV 色彩选择对话框，含色相条 + HSV 方块 + 常用色速取网格（[TTyColorGrid](colorgrid.md)，两行 32 色：16 色 VGA 基础色 + 常用色调 + 灰阶）+ RGB / CMYK / Hex 输入框 + Alpha 滑块，所有通道全双向同步：点色块与拖色相条一样，走同一条提交路径。

色相与饱和度是**取色器自己的状态**，不是每次都从 RGB 反推的：黑色与灰色本身没有色相（RGB→HSV 会按惯例给 0＝红），只有当颜色来自取色器**之外**（Hex / RGB / CMYK 输入框、色块、`Execute` 预置值）且反推出的值确实有意义时才回填。所以把明度拖到底再拖回来，色相不会跳成红色。
