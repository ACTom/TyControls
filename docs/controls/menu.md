...
- **右到左镜像：方向从哪里来。** 菜单栏由 `TTyMenuBar.BiDiMode` 决定；上下文菜单由**弹出它的那个控件**决定（LCL 在弹出前把该控件写进 `TPopupMenu.PopupComponent`，没有时回落到 `Owner`）——`TPopupMenu` 是组件不是控件，自己没有 `BiDiMode`，而弹出体那个窗体是本库用 `TForm.CreateNew` 造的，不会从应用主窗口继承任何东西。方向沿着级联一路往下传：一个菜单从头到尾只按一个方向读。
- **右到左镜像：菜单栏。** 顶级格子从右往左铺，`RightJustify` 那一组改为贴住左边缘。`TopLeft` 是唯一做镜像的地方，绘制、命中（`TopAtX`）与下拉锚点全都读它，因此不可能"画在右边、点在左边"。`FitWidth`（`AutoSizeWidth` 用的内容宽度）刻意读**未镜像**的那一份：反射是绕 `Width` 做的，而 `Width` 正是它要算出来的东西。
- **右到左镜像：弹出体。** 勾选 / 图标槽移到行的右端，快捷键文字与子菜单箭头移到左端，**箭头本身也转向**（左槽里放右向箭头是典型的半镜像，它指着子菜单不会出现的方向）。装饰性侧边色带换到另一端，行内容跟着让开。行内没有任何 x 方向的命中判定（`RowAtY` 只看 y），这正是"移动这些槽位是安全改动"的原因。
- **右到左镜像：弹出方向。** 下拉菜单改为把**右**边缘对齐锚点右侧；子菜单朝**左**级联，贴到屏幕左缘时才翻回右侧。`TTyPopupMenu.Alignment` 按阅读序解释（`paLeft` = 对齐阅读起点），锚点位移由纯函数 `TyPopupAnchorShift` 给出——那是个只有符号的一行式，而符号错了截图仍然完全正确。
- **右到左镜像：方向键对调。** 镜像后 **← 是进入子菜单、→ 是返回父级**（`plans/2026-08-04-rtl-mirroring-scope.md` §6.3 第 4 条：列表 / 网格 / 标签页 / 菜单里的方向键是**布局**方向，要翻；文本编辑里的不翻）。漏掉这一条不是外观问题——右到左用户将完全无法用键盘打开子菜单，而任何截图都看不出来。上下键与 Home/End 不受影响。见 [KNOWN_GAPS.md](../KNOWN_GAPS.md#bidirectional-right-to-left-text)。
