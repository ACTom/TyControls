...
- **右到左镜像：方向键对调。** 镜像后 **← 是进入子菜单、→ 是返回父级**（`plans/2026-08-04-rtl-mirroring-scope.md` §6.3 第 4 条：列表 / 网格 / 标签页 / 菜单里的方向键是**布局**方向，要翻；文本编辑里的不翻）。漏掉这一条不是外观问题——右到左用户将完全无法用键盘打开子菜单，而任何截图都看不出来。上下键与 Home/End 不受影响。见 [rtl.md](../rtl.md)。
