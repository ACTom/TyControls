...
**设计期**：双击控件（或右键「编辑选项...」）打开专用的**选项结构编辑器**——一棵树看全整个嵌套模型（任意深度），右侧「加节点 / 加子节点 / 删除 / 上移 / 下移」原地增删排序（加节点插在选中行正下方）。树里选中任何选项，Object Inspector 直接显示它的 `Caption`/`Enabled`/`Tag` 当场改；窗口非模态。标准集合编辑器一次只显示一层（每个分支开一个窗），仍可从 OI 里 `Nodes`/`Children` 的省略号进入。
