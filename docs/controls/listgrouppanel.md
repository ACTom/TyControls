...
- `Collapsed := True` 把整个面板收窄成图标轨道：条目和分组头只画居中图标,选中胶囊保留;展开时恢复收缩前的宽度。面板通常 `Align = alLeft`,宽度变化会带动整窗重排——这就是"给内容让位"。
- 轨道上的层级区分：每个分组（第一个除外）上方画一条内缩的短分隔线,把轨道切成段——段首就是分组头。无图标的行画标题**首字**（正确按 UTF-8 取整个字,中文标题显示首汉字）,不再留成空白格。分隔线用主题的边框色,无边框主题回落到低透明度文字色。
