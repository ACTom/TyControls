...
- **宿主要给角留空:** 角标坐在宿主**角内**、压在宿主上(窗口化宿主自带句柄,溢到宿主矩形外的部分会被句柄盖掉,所以角标只能待在里面)。若宿主的内容一直顶到那个角——典型是**居中的长标题**顶到右上角——角标就会压住它。让那个角空出来:把宿主放宽,或改用更小的角标(圆点 / 短数字)。反例:窄按钮上一颗 `99+`,标题居中铺满,右上角标压掉标题末字。
