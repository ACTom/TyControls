...
```pascal
// 流是导出核心：不落盘直接把图喂给报表/网络。AFormat 用 BGRABitmap 的
// TBGRAImageFormat（ifPng / ifJpeg / ifBmp / ifTiff……，uses BGRABitmapTypes）。
procedure SaveToStream(AStream: TStream; AFormat: TBGRAImageFormat);
procedure SaveToStream(AStream: TStream; AFormat: TBGRAImageFormat; AWidth, AHeight: Integer);

// 文件封装：不带格式按扩展名（.png/.bmp/.jpg/.jpeg/.tif）；带格式则显式格式说了算，
// 文件名叫什么都行。
procedure SaveToFile(const AFileName: string);
procedure SaveToFile(const AFileName: string; AWidth, AHeight: Integer);
procedure SaveToFile(const AFileName: string; AFormat: TBGRAImageFormat);
procedure SaveToFile(const AFileName: string; AFormat: TBGRAImageFormat; AWidth, AHeight: Integer);
```

不带尺寸按控件当前尺寸导出；带尺寸的重载按给定宽高重新排版（小图表可以导出成大图），文字保持控件的 PPI。背景取当前主题，导出图不透明。
