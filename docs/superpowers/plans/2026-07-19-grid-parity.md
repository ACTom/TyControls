...
## 合并前遗留:固定行不画内容 + 右/下冻结(2026-07-19 记)

**先修这个,别直接上右/下冻结。**

### 已证实的 bug:顶部固定行只占高度、一个字都不画
`FixedRows := 1` 之后那一条带是空白的(示例基础页勾「固定行」就能看到)。
原因:`TyGridVisibleRows` 显式把固定行排除在窗口之外
(`if AFirst < M.FixedRows then AFirst := M.FixedRows`),而**全文件没有第二个
渲染固定行的循环**。10 处逐行循环全都只画正文窗口。
守卫写法:渲染后数固定行那一格矩形里的墨(实测 0)。

### 为什么右/下冻结不是"两个属性"
之前把它当成最便宜的一项是**估错了**。九宫格切分(`TyGridPaneRect`)确实做完了,
但定位/命中层完全不认右与下:
`TyGridRowRect` 只 pin `ARow < FixedRows`;`TyGridVisibleRows`、`TyGridRowAt`、
`ColumnLeftPx`、`ColumnAtX`、滚动范围、以及脏区快路径的平移带(写死 `[FrozenTop, ClientH)`)
都要一起改。

### 结果(2026-07-19 已全部完成)
- **固定行不画内容:已修**。绘制改走 `TyGridDrawSlots` / `TyGridRowAtSlot` 槽位,
  `TyGridVisibleRows` 语义原样不动(寻址/滚动/命中和多条测试都依赖它)。
  另一半是 `CellPane` 把每一行也分窗格 —— 从前所有行都算正文窗格,而正文窗格
  从冻结带**之下**才开始,固定行的可见矩形恒为空。
- **`FixedRowsBottom` / `FixedColsRight`:已做**,含命中反演与脏区快路径的平移带边界。
- 顺带修掉两处同类裁剪 bug:行号与横格线用一把大裁剪而不是按窗格裁,
  滚到冻结带底下的行会把号码/线画进冻结带。单元格内容一直是安全的
  (`CellVisibleRect` 与窗格求交),这两条路径当初绕过了它。

### 当初试过一半又回退的走法(留作教训)
把逐行循环从"行区间"改成"**槽位区间**",槽位映射收口在一个
`TyGridRowAtSlot(ASlot, M)` 里:槽位 = 顶部固定行 + 正文窗口 + 底部固定行。
这样"固定行也要画""底部固定行也要画"都只写一遍,10 处循环各改一行,
而右/下冻结几乎是顺带的。

**回退原因**:机械改写 10 处循环时把渲染改坏了(基础单元格文字都不画,16 条红)。
这个重构值得做,但要单独一次、逐处手改并逐处跑测试,不能批量正则替换。
现有的 `TyGridVisibleRows` 语义(返回**行**区间)被多条测试直接断言,
改语义时那些测试要一起改 —— 这也是批量替换会翻车的原因之一。
