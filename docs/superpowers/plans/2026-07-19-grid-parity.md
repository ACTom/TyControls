...
- [ ] **兼容步**:先加新字段并让 `FrozenLeft := FrozenW`、`FrozenTop := FrozenH`、
      `HeaderBands := [HeaderH]` 等价映射,**保留旧字段名做只读别名**,
      跑全套件确认 0 回归(证明重构没碰坏行为 —— 与当初引入行序间接层同一手法)。
- [ ] 写失败测试 `TestNinePanesTileTheViewport`:四向冻结都非零时,
      3×3 共 9 个窗格必须**精确铺满视口**,用面积守恒证明(与现有四窗格那条同样的证法)。
- [ ] 跑,确认红。
- [ ] 实现 `TyGridPaneRect` 的 3×3 版本(枚举扩成 9 值,旧的 4 个值保留语义)。
- [ ] 跑,确认绿。
- [ ] 写失败测试 `TestHeaderBandsStackAndSumToFrozenTop`:
      `HeaderBands = [24, 20]` 时,第 0 级带在 y∈[0,24),第 1 级在 y∈[24,44),
      且固定行从 44 起算。
- [ ] 跑红 → 实现 → 跑绿。
- [ ] 写失败测试 `TestGridLineWidthShiftsCellBoundaries`:线宽 1→3 时,
      第 1 列的单元格左边界右移,且**命中仍是矩形的逆**(逐像素反查)。
- [ ] 跑红 → 实现 → 跑绿。
- [ ] 变异:① 让 `HeaderBands` 只取第一项 → 多级表头测试红;
      ② 让线宽不参与分配 → 边界测试红。各自恢复。
- [ ] 删掉兼容别名,把 `Grid.pas` 全部改用新字段;跑全套件确认 0 回归。
- [ ] **B2 收工**:收工条件 1-7。
